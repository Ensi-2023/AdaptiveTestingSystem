...
{
    public class Command_SetQuestionThisTesting:Commands
    {
        public override void Execut(string json, InternetClient client)
        {
            try
            {
                var obj = JsonSerializer.Deserialize<List<Data_Question>>(json);
                if (obj == null) return;

                Application.Current.Dispatcher.Invoke(() =>
                {

                    var guiUID = (_Main.Instance.MainBody.Children[0] as View_BodyApplication).Main.Children[0] as GUI_TestViewer;
                    if (guiUID != null)
                    {
